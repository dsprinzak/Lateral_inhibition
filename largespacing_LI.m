...
function [yout,tout,params,F] = largespacing_LI(params)

% largespacing_LI simulates lateral inhibition in a hexagonal lattice with 
% next nearest neighbors interactions between cells. 
...
Tmax=60; tspan=[0 Tmax]; % set time for simulation
...
w=1/18; % weight for interactions

% calculating the next nearest neighbor connectivity matrix

for s=1:k
    % find the neighbors of cell s
    kneighbor=findneighborhex(s,P,Q); 
    nn_neighbor=kneighbor;
    
    % find the neighbors of the neighbors of cell s
    for i=1:length(kneighbor)
        nn_neighbor=[nn_neighbor; findneighborhex(kneighbor(i),P,Q)]; 
    end
    
    % find all the unique neighbors of cell s
    nn_neighbor=unique(nn_neighbor); 
    
    for r=1:length(nn_neighbor);
        M(s,nn_neighbor(r))=w;
    end
    M(s,s)=0; % removing cell s from the connectivity matrix
end

function y0=getIC(params,k)

U=rand(k,1) - 1/2; % uniform random distribution
