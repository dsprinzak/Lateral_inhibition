...
function [yout,tout,params,F] = morphogen_LI(params)
% morphogen_LI simulates lateral inhibition in a hexagonal lattice. 
% The morphogen is introduced through a gradient on betaD. 
...
% YOUT is a matrix containing the numerical solution for each variable 
% for each time point. Each row in YOUT is a vector of the size of TOUT. 
...
% apply morphogen controlling betaD
% params.l is the lengthscale of morphogen 
params.l=1.5;
Morph=getMorph(params.l,P,Q); % gets an exponential morphogen profile with lengthscale params.l
params.betaD=params.betaD*Morph; % params.betaD becomes a vector describing the local production of Delta controlled by a morphogen
...
Dneighbor=M*y(1:k);% average Delta levels in the neighboring cells

dD = nu * (betaD.*1./(1 + R.^h)-D); % differential equation describing Delta levels
dR = betaR*Dneighbor.^m./(1 + Dneighbor.^m)-R; % differential equation describing Repressor level
...
function Morph=getMorph(l,P,Q)

center=[floor(P/2) floor(Q/2)];

MorpPQ=zeros(P,Q);
Morph=zeros(P*Q,1);
for p=1:P
    for q=1:Q
        distpq=sqrt((((p-center(1))/l)^2)+(((q-center(2))/l)^2));
        MorphPQ(p,q)=exp(-distpq);
        ind=pq2ind(p,q, P);
        Morph(ind)=MorphPQ(p,q);
    end
end
